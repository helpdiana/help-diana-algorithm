...
def Trans(path,filename):
    ...
    trans_total_imsi = []
    #img별로 접근
    for img in range(0,len(json_data['diagnose_bf'])):
        #img안의 문장별로 접근
        for sent in range(0,len(json_data['diagnose_bf'][img])):

            try :
                #papago 해석
                sentences = json_data['diagnose_bf'][img][sent][0]

                #papago api 유료ver
                translatedText = papago(sentences,s1 = 'ko', s2 = 'en')
                #print(mid_translatedText)
                #print()
                #translatedText = papago(translatedText,s1 = 'en', s2 = 'ko')
    ...
    trans_dic = {
    "diagnose_eng_af": trans_total_imsi
    }

    #papago 번역 결과
    with open(f"{path}/eng_trans_{filename}", "w", encoding='utf-8') as pf:
